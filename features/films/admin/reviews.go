...
func EditFilmReviewHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		film, err := films.GetFilmByID(r.Context(), review.FilmID)
		if err != nil {
			http.Error(w, "Film not found", http.StatusNotFound)
			return
		}

		data := filmtemplates.EditFilmReviewData{
			ID:          review.ID,
			FilmID:      film.ID,
		...
		if err := filmtemplates.RenderEditFilmReview(w, data); err != nil {
...
func CreateFilmReviewHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = films.GetFilmByID(r.Context(), id)
		if err != nil {
			http.Error(w, "Film not found", http.StatusNotFound)
			return
		}

		reviewID, err := films.CreateFilmReview(r.Context(), id, 0, time.Now(), false, false, false, "")
		if err != nil {
