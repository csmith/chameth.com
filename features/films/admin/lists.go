...
func ListFilmListsHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		drafts, err := films.GetDraftFilmLists(r.Context())
		if err != nil {
		...
		lists, err := films.GetAllFilmLists(r.Context())
		if err != nil {
		...
		draftSummaries := make([]filmtemplates.FilmListSummary, len(drafts))
		for i, list := range drafts {
			draftSummaries[i] = filmtemplates.FilmListSummary{
				ID:        list.ID,
				Title:     list.Title,
				Path:      list.Path,
		...
		listSummaries := make([]filmtemplates.FilmListSummary, len(lists))
		for i, list := range lists {
			listSummaries[i] = filmtemplates.FilmListSummary{
				ID:        list.ID,
				Title:     list.Title,
				Path:      list.Path,
				Published: list.Published,
		...
		data := filmtemplates.ListFilmListsData{
			Drafts: draftSummaries,
			Lists:  listSummaries,
		}

		if err := filmtemplates.RenderListFilmLists(w, data); err != nil {
...
func CreateFilmListHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		id, err := films.CreateFilmList(r.Context(), path, name, "")
		if err != nil {
...
func EditFilmListHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		list, entries, err := films.GetFilmListWithEntries(r.Context(), id)
		if err != nil {
		...
		entryItems := make([]filmtemplates.FilmListEntryItem, len(entries))
		...
		for i, entry := range entries {
			...
			entryItems[i] = filmtemplates.FilmListEntryItem{
				EntryID:  entry.ID,
		...
		availableFilms := make([]films.Film, 0, len(allFilms))
		...
		filmOptions := make([]filmtemplates.FilmOption, len(availableFilms))
		for i, film := range availableFilms {
			year := ""
			if film.Year != nil {
				year = strconv.Itoa(*film.Year)
			}
			filmOptions[i] = filmtemplates.FilmOption{
				ID:    film.ID,
				Title: film.Title,
				Year:  year,
			}
		...
		data := filmtemplates.EditFilmListData{
			ID:          list.ID,
			Title:       list.Title,
		...
		if err := filmtemplates.RenderEditFilmList(w, data); err != nil {
...
func UpdateFilmListHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := films.UpdateFilmList(r.Context(), id, path, title, description, published); err != nil {
...
func AddFilmToListHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		position, err := films.GetNextPosition(r.Context(), id)
		if err != nil {
		...
		_, err = films.AddFilmToList(r.Context(), id, filmID, position)
		if err != nil {
...
func RemoveFilmFromListHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := films.RemoveFilmFromList(r.Context(), entryID); err != nil {
...
func UpdateEntryPositionHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := films.UpdateEntryPosition(r.Context(), entryID, position); err != nil {
...
func ReorderFilmListEntriesHandler() func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := films.ReorderFilmListEntries(r.Context(), id); err != nil {
