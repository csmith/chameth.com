...
// BaseURL is the Magic Meters daemon's address on the tailnet. The client
...
const BaseURL = "https://mm.yak-wall.ts.net"

// ErrNotFound is returned when an endpoint reports 404, e.g. a game's box
// art that has not been fetched yet.
...
func (c *Client) get(ctx context.Context, path string, query url.Values) ([]byte, http.Header, error) {
	...
	if res.StatusCode >= 400 {
		slog.Warn("Magic Meters request failed", "path", path, "status", res.StatusCode, "response", string(b))
